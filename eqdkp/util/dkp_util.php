...
class dkp_util
{
	...
	public function logAction($action)
	{
		...
		$sql = 'INSERT INTO phpbb_eqdkp_log ' . $this->db->sql_build_array('INSERT', $data);
	...
	public function adjustDKP($user_id, $desc, $by, $value, $second_pool)
	{
		...
		$sql = 'INSERT INTO phpbb_eqdkp_adjustment ' . $this->db->sql_build_array('INSERT', $data);
		$this->db->sql_query($sql);
	}
	...
	public function delAdjustment($adj_id)
	{
		...
		$sql = 'DELETE FROM phpbb_eqdkp_adjustment where adj_id = ' . (int) $adj_id;
		$this->db->sql_query($sql);
	}
	...
	public function getDKPBreakdown($user_id, $role = 0)
	{
		...
		$sql = 'SELECT
			...
			LEFT OUTER JOIN phpbb_eqdkp_raid_attendance a
				ON (a.raid_id = r.raid_id AND
						a.user_id = ' . (int) $user_id . ' AND
						a.char_role = ' . (int) $char_role . ')
			LEFT OUTER JOIN phpbb_eqdkp_characters c
		...
		$sql = 'SELECT
				...
				lu.name as itemname,
				u.username as entered_by
			FROM phpbb_eqdkp_raid_loot l
			JOIN phpbb_users u
				ON (u.user_id = l.entered_by)
			JOIN phpbb_eqdkp_raid r
	...
	function getDKP($user_id, $role = 0, $include_open = 1)
	{
		...
		$sql = 'SELECT
			...
			FROM phpbb_eqdkp_raid_attendance
			WHERE user_id = ' . (int) $user_id . '
		...
		if ($include_open == 1) {
			...
			$sql = 'SELECT
				...
				FROM phpbb_eqdkp_raid as t1
				LEFT JOIN phpbb_eqdkp_raid_attendance as t2
			...
			if (isset($row['char_times'])) {
				if ($role != 2 || $row['seconds_earn'] == 1) {
					$ppt = $role == 2 ? $this->config['eqdkp_second_dkp_per_tick'] : $this->config['eqdkp_main_dkp_per_tick'];
					if ($row['double_dkp'] == 1) {
						$ppt += $ppt;
					}
		...
		if ($include_open == 1) {
			...
			foreach ($rows as $row) {
				if ($row['seconds_earn'] !=0 || $row['char_role'] == 2) {
					$ppt = ($row['char_role'] == 2 ? $this->config['eqdkp_main_dkp_per_tick'] : $this->config['eqdkp_second_dkp_per_tick']);
					if ($row['double_dkp'] == 1) {
						$ppt += $ppt;
					}
	...
	public function getAllStartEnd($char_role = 2)
	{
		...
		$sql = 'SELECT
			...
			FROM phpbb_eqdkp_raid_attendance as t1
			JOIN phpbb_eqdkp_raid as t2
				ON (t2.raid_id = t1.raid_id)
	...
	public function getStartDate($user_id)
	{
		$sql = 'SELECT
			CASE
				WHEN c.eqdkp_start_date IS NOT NULL
				THEN c.eqdkp_start_date
				ELSE MIN(r.day)
				END as first
			FROM phpbb_users c
			JOIN phpbb_eqdkp_raid_attendance a
	...
	public function getAllPerc($char_role = 2)
	{
		...
		$sql = 'SELECT
			c.user_id, CASE WHEN c.eqdkp_start_date IS NOT NULL THEN c.eqdkp_start_date ELSE min(r.day) END as first
			FROM (
				SELECT u.user_id, u.eqdkp_start_date, c.char_id
				FROM phpbb_users u
				JOIN phpbb_eqdkp_characters c
			...
			LEFT OUTER JOIN phpbb_eqdkp_raid r
				ON (r.raid_id = a.raid_id)
			GROUP BY c.user_id';
		$result = $this->db->sql_query($sql);
		$rows = $this->db->sql_fetchrowset($result);

...
class dkp_decay extends \phpbb\cron\task\base
{
	...
	public function run()
	{
		...
		$this->config->set('eqdkp_decay_last_run', time(), false);

		error_log("Running weekly DKP decay cron job");

		// Run your cron actions here...
		$decay_perc = $this->config['eqdkp_decay_perc'];
	...
	{
		return $this->config['eqdkp_decay_last_run'] < time() - $this->config['eqdkp_decay_interval'];
	}
}

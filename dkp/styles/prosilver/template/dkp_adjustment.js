...
function submitAll( event ) {
	...
	$.ajax({
		type: 'post',
		dataType: 'json',
		url: czphpbbDKPbulkadj,
		data: JSON.stringify($.czphpbbDKP.adjustments),
		success: function (data) {
			$.czphpbbDKP.adjustments.length = 0;
			displayPreview();
			},
		error: function (data) {
...
function displayPreview() {
	...
	if ($.czphpbbDKP.adjustments.length > 0) {
		// two column display
		var midpoint = Math.floor($.czphpbbDKP.adjustments.length/2);
		var div = $('<div/>', { style: 'width: 395px; float: left; margin: 2px;' });
		var table = createTable(['Character', 'Description', 'Amount', 'Pool']);
		div.append(table);
		for (var i = 0; i < $.czphpbbDKP.adjustments.length; i++) {
			if (i == midpoint && i != 0) {
				// setup a new div
			...
			var user_id = $.czphpbbDKP.adjustments[i].user_id;
			var desc = $.czphpbbDKP.adjustments[i].desc;
			var pool = $.czphpbbDKP.adjustments[i].pool;
			var value = $.czphpbbDKP.adjustments[i].value;
			var adjRow = $('<tr/>', {
					'class': rowClass,
					'data-adjid': i,
					});
...
function adjRemoveClicked( event ) {
	...
	$.czphpbbDKP.adjustments.splice(adjID, 1);
	displayPreview();
...
function calcValue(adjvalue, adjid, pool) {
	...
	var cVal = $.czphpbbDKP.dkp[adjid][pool];
...
function getUserID(charName) {
	...
	var userIDs = Object.keys($.czphpbbDKP.charlist);
	for (var i = 0; i < userIDs.length; i++) {
		var uid = userIDs[i];
		for (var x = 0; x < $.czphpbbDKP.charlist[uid].length; x++) {
			if ($.czphpbbDKP.charlist[uid][x].name.toLowerCase() == query) {
...
function addAdjustment( event ) {
	...
	if ($('#adjfile').val() && $.czphpbbDKP.fileData.length > 0) {
		...
		for (var i = 0; i < $.czphpbbDKP.fileData.length; i++) {
			var uid = $.czphpbbDKP.fileData[i].user_id;
			var fileValue = $.czphpbbDKP.fileData[i].value;
			if (!adjvalue && !fileValue) {
				alert('No adjustment value set in the form or the file for user: ' + $.czphpbbDKP.fileData[i].name);
				return;
			}
			if (adjmain) {
		...
		if (!adjid && !adjname) {
			...
			var userIDs = Object.keys($.czphpbbDKP.dkp);
			for (var i = 0; i < userIDs.length; i++) {
				var uid = userIDs[i];
				if (adjmain) {
...
function searchChar(request, response) {
	...
	for (var i = 0; i < userIDs.length; i++) {
		var uid = userIDs[i];
		for (var x = 0; x < $.czphpbbDKP.charlist[uid].length; x++) {
			if ($.czphpbbDKP.charlist[uid][x].name.toLowerCase().startsWith(query)) {
				// have a match, add to data
				data.push({'label': $.czphpbbDKP.charlist[uid][x].name, 'value': uid});
			}
		}
	}
	response(data);
...
function adjReadyFn( jQuery ) {
	...
	$.czphpbbDKP.adjustments = new Array();
	$.czphpbbDKP.fileData = new Array();

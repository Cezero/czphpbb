...
namespace bum\dkp\controller;

use bum\dkp\util\eq_const;
...
class main
{
	...
	public function __construct(\phpbb\config\config $config, \phpbb\db\driver\driver_interface $db, \phpbb\controller\helper $helper, \phpbb\template\template $template, ContainerInterface $phpbb_container, \phpbb\request\request $request, \phpbb\user $user, $table_prefix)
	{
		...
		$this->char_table = $table_prefix . 'bum_dkp_characters';
		$this->raid_table = $table_prefix . 'bum_dkp_raid';
		$this->loot_table = $table_prefix . 'bum_dkp_raid_loot';
		$this->attendance_table = $table_prefix . 'bum_dkp_raid_attendance';
	}
	...
	public function handle($name)
	{
		...
		$characterlist = $this->phpbb_container->get('bum.dkp.util.characterlist');
		...
		$this->template->assign_vars(array(
					'U_ITEM_QUERY' => $this->helper->route('bum_dkp_controller', array('name' => 'item_query')),
					'U_CHAR_QUERY' => $this->helper->route('bum_dkp_controller', array('name' => 'char_query')),
					'U_AWARD_ITEM' => $this->helper->route('bum_dkp_controller', array('name' => 'award_item')),
					'U_DEL_ITEM' => $this->helper->route('bum_dkp_controller', array('name' => 'remove_item')),
					'U_DEL_ADJ' => $this->helper->route('bum_dkp_controller', array('name' => 'deladj')),
					'U_ADD_ADJ' => $this->helper->route('bum_dkp_controller', array('name' => 'addadj')),
					'U_BULK_ADJ' => $this->helper->route('bum_dkp_controller', array('name' => 'bulkadj')),
					'U_CHARLIST' => $this->helper->route('bum_dkp_controller', array('name' => 'charlist')),
					'U_FULLCHARLIST' => $this->helper->route('bum_dkp_controller', array('name' => 'fullcharlist')),
					'U_GETALLDKP' => $this->helper->route('bum_dkp_controller', array('name' => 'getalldkp')),
					'U_ITEMLIST' => $this->helper->route('bum_dkp_controller', array('name' => 'item_list')),
					'U_VIEW_ITEM' => $this->helper->route('bum_dkp_controller', array('name' => 'viewitem')),
					'V_WEEKEND_START' => $this->config['bum_dkp_weekend_start'],
					'V_WEEKDAY_START' => $this->config['bum_dkp_weekday_start'],
					'V_MAINTICK' => $this->config['bum_dkp_main_dkp_per_tick'],
					'V_SECONDTICK' => $this->config['bum_dkp_second_dkp_per_tick'],
					'U_ROSTER' => $this->helper->route('bum_dkp_controller', array('name' => 'roster')),
					'U_RAID_LIST' => $this->helper->route('bum_dkp_controller', array('name' => 'raidlist')),
					));
        ...
        if (!isset($sort_func[$sort_key]))
				...
				$page_title = $this->user->lang['BUM_DKP_DETAILS'] . ': ' . $char_info['char_name'];
				...
				foreach ($type_map as $type => $type_str) {
					...
					if (!$filter_type || $filter_type == $type) {
						...
						if ($type == 'rollover') {
							if ($data) {
								$dkp_rows[] = array('date' => 0, 'data' => array(), 'type' => $type_str, 'amnt' => $data, 'enteredby' => 'Bumamgar');
							}
						} else {
							foreach ($data as $row) {
								$dkp_rows[] = array(
				...
				$pagination_url = append_sid($this->helper->route('bum_dkp_controller', array('name' => 'dkpdetail')), implode('&amp;', $params));
				$sort_url = append_sid($this->helper->route('bum_dkp_controller', array('name' => 'dkpdetail')), implode('&amp;', $sort_params));
				...
				foreach ($type_map as $type => $desc) {
					$this->template->assign_block_vars('filter_type', array(
								...
								'U_SORT' => append_sid($this->helper->route('bum_dkp_controller', array('name' => 'dkpdetail')), $u_filter_type_params . 'filter_type=' . $type),
								));
				}
				...
				foreach ($rows as $dkp_entry) {
					...
					if ($dkp_entry['type'] == 'Raid') {
						...
						$block_vars['U_VIEW_EVENT'] = $this->helper->route('bum_dkp_controller', array('name' => 'viewraid', 'raidid' => (int) $dkp_entry['data']['raid_id']));
						$block_vars['U_EDIT_EVENT'] = $this->helper->route('bum_dkp_controller', array('name' => 'addraid', 'raidid' => (int) $dkp_entry['data']['raid_id']));
						...
						$block_vars['U_ITEMINFO'] = $this->helper->route('bum_dkp_controller', array('name' => 'viewitem', 'lucyid' => $dkp_entry['data']['lucy_id']));
						$block_vars['U_EDIT_EVENT'] = $this->helper->route('bum_dkp_controller', array('name' => 'addraid', 'raidid' => (int) $dkp_entry['data']['raid_id']));
						$block_vars['S_EDIT_EVENT'] = 'Edit Raid';
				...
				$page_title = $this->user->lang['BUM_DKP_RAID_LIST'];
        ...
        $sort_dir = $this->request->variable('sd', 'd');
				...
				$pagination_url = append_sid($this->helper->route('bum_dkp_controller', array('name' => 'raidlist')), implode('&amp;', $params));
				$sort_url = append_sid($this->helper->route('bum_dkp_controller', array('name' => 'raidlist')), implode('&amp;', $sort_params));
	      ...
	      $this->db->sql_freeresult($result);
				...
				foreach ($rows as $raid_entry) {
					$ppt = $this->config['bum_dkp_main_dkp_per_tick'];
					if ($raid_entry['double_dkp'])
						$ppt += $ppt;

					$this->template->assign_block_vars('raidrow', array(
								'DATE' => date('Y-m-d', $raid_entry['day']),
								'U_VIEW_RAID' => $this->helper->route('bum_dkp_controller', array('name' => 'viewraid', 'raidid' => (int) $raid_entry['raid_id'])),
								'U_EDIT_RAID' => $this->helper->route('bum_dkp_controller', array('name' => 'addraid', 'raidid' => (int) $raid_entry['raid_id'])),
								'U_DEL_RAID' => $this->helper->route('bum_dkp_controller', array('name' => 'delraid', 'raidid' => (int) $raid_entry['raid_id'])),
				...
				$page_title = $this->user->lang['BUM_DKP_ROSTER_TITLE'];
				...
				*/
				...
				$pagination_url = append_sid($this->helper->route('bum_dkp_controller', array('name' => 'roster')), implode('&amp;', $params));
				$sort_url = append_sid($this->helper->route('bum_dkp_controller', array('name' => 'roster')), implode('&amp;', $sort_params));
				...
				foreach ($filter_classes as $class => $desc) {
					$this->template->assign_block_vars('filter_class', array(
								...
								'U_SORT' => append_sid($this->helper->route('bum_dkp_controller', array('name' => 'roster')), $u_filter_class_params . 'filter_class=' . $class),
							));
				}
				...
				foreach ($rows as $roster_entry) {
					...
					$this->template->assign_block_vars('characterrow', array(
								...
								'U_MDKP_DETAIL' => $this->helper->route('bum_dkp_controller', array('name' => 'dkpdetail', 'role' => 0, 'userid' => $user_id)),
								'U_SDKP_DETAIL' => $this->helper->route('bum_dkp_controller', array('name' => 'dkpdetail', 'role' => 1, 'userid' => $user_id)),
								));
				}
				...
				$page_title = $this->user->lang['BUM_DKP_ADJUSTMENT_TITLE'];

				add_form_key('bum/dkp/adjustment');
			    ...
			    $this->db->sql_freeresult($result);
					...
					$sql = 'delete from phpbb_bum_dkp_raid where raid_id = '.(int) $raid_id;
					$this->db->sql_query($sql);
				...
				$this->db->sql_freeresult($result);
				$page_title = $this->user->lang('BUM_DKP_VIEW_RAID_TITLE', $raid_summary['description'] );
				// get loot
				$sql = 'SELECT rl.*, li.name, c1.char_name as awarded, c1.role, c2.char_name as main
					FROM ' . $this->loot_table . ' as rl
					JOIN lucy_itemlist as li
					...
					foreach (array_chunk($loot_rows, $midpoint, true) as $col) {
						...
						foreach ($col as $row) {
							...
							$this->template->assign_block_vars('loot.col.row', array(
										...
										'NAME' => $row['name'],
										'AWARDED' => $award_str,
										'U_VIEW_ITEM' => $this->helper->route('bum_dkp_controller', array('name' => 'viewitem', 'lucyid' => (int) $row['lucy_id'])),
										));
						}
					}
				}
				...
				$sql = 'SELECT u.user_id, c2.char_name main, c.char_name, c.role
					FROM phpbb_users u
						...
						JOIN phpbb_bum_dkp_characters c2
				...
				$sql = 'SELECT a.user_id, c2.char_name main, c.char_name, a.char_role, a.char_times, a.earned_dkp, a.ticks
					FROM phpbb_bum_dkp_raid_attendance a
						JOIN phpbb_bum_dkp_characters c
              ...
              'SECONDS_CHECKED' => $raid_summary['seconds_earn'] ? ' checked' : '',
				...
				$this->db->sql_freeresult($result);
				$page_title = $this->user->lang('BUM_DKP_VIEW_ITEM_TITLE', $item_name );

				// get all drop info
				$sql = 'SELECT r.day, r.description, rl.user_id, c1.char_name as awarded, c1.role, c2.char_name as main, rl.cost, rl.second_pool
					FROM phpbb_bum_dkp_raid_loot as rl
						JOIN phpbb_bum_dkp_raid as r
							ON (r.raid_id = rl.raid_id)
						JOIN phpbb_bum_dkp_characters as c1
							ON (c1.user_id = rl.user_id and c1.char_id = rl.char_id)
						JOIN phpbb_bum_dkp_characters as c2
							ON (c2.user_id = rl.user_id and c2.role = 2)
				...
				$sql = 'SELECT c.role, count(*) cnt, avg(rl.cost) avgc, min(rl.cost) minc, max(rl.cost) maxc
					FROM phpbb_bum_dkp_raid_loot rl
						JOIN phpbb_bum_dkp_characters c
				...
				$raid_desc = '';
				$raid_date = '';
				$seconds_checked = $this->config['bum_dkp_seconds_earn_dkp'] ? 1 : 0;
				$double_dkp = 0;
				$merge_dump = 0;
				...
				if ($this->request->is_set_post('addraid')) {
					if (!check_form_key('bum/dkp/addraid')) {
						trigger_error($this->user->lang['FORM_INVALID']);
					}

					$button_label = 'Submit Changes';
					$raid_desc = $this->request->variable('bum_dkp_raiddescription', $raid_desc);
					if (!$raid_desc) {
						trigger_error('Raid description must be set!');
					}

					$raid_date = $this->request->variable('bum_dkp_raiddate', $raid_date);
					$raid_start = $this->request->variable('bum_dkp_raidstart', $raid_start);
					$raid_end = $this->request->variable('bum_dkp_raidend', $raid_end);
					$seconds_checked = $this->request->variable('bum_dkp_seconds_earn', $seconds_checked);
					$double_dkp = $this->request->variable('bum_dkp_double', $double_dkp);
					$merge_dump = $this->request->variable('bum_dkp_merge_dump', $merge_dump);
					...
					if ($raid_start < 0) {
						if (date('N', strtotime($raid_date)) >= 6) {
							$raid_start = $this->config['bum_dkp_weekend_start'];
						} else {
							$raid_start = $this->config['bum_dkp_weekday_start'];
						}
					}
							...
							if (!empty($user_times[$role][$user_id])) {
								...
								if ($raid_end > -1) {
									if ($role == 2 || ($role == 1 && $seconds_checked == 1)) {
										$ppt = $role == 2 ? $this->config['bum_dkp_main_dkp_per_tick'] : $this->config['bum_dkp_second_dkp_per_tick'];
										if ($double_dkp) {
											$ppt += $ppt;
										}
				...
				if ($raid_id > -1) {
					$page_title = $this->user->lang['BUM_DKP_EDIT_RAID_TITLE'];
				} else {
					$page_title = $this->user->lang['BUM_DKP_ADD_RAID_TITLE'];
				}

				add_form_key('bum/dkp/addraid');
			...
			if ($is_officer) {
				...
				$sql = 'select raid_id from phpbb_bum_dkp_raid where end < 0';
				$this->db->sql_query($sql);
				...
				$this->template->assign_vars(array(
							...
							'U_ADD_RAID' => $this->helper->route('bum_dkp_controller', $add_raid_rt),
							'ADD_RAID_LABEL' => $active_id ? $this->user->lang['OFFICER_EDIT_ACTIVE'] : $this->user->lang['OFFICER_ADD_RAID'],
							'U_ADJUSTMENTS' => $this->helper->route('bum_dkp_controller', array('name' => 'adjustment')),
							));
			}
